...
import com.trade.master.core.entity.*;
...
import com.trade.master.core.repository.BotTradeConfigRepository;
...
public class PoloniexTradeConfigBot implements PolBot {
        ...
        for (BotTradeConfig botTradeConfig : botTradeConfigs) {
            ...
            if (highestSellPrice.compareTo(botTradeConfig.getStopLossPrice()) <= 0) {
                ...
                if (coinBalance.compareTo(BigDecimal.valueOf(0)) > 0) {
                    Order order = new Order(currPair, SELL_ACTION, lowestBuyPrice.multiply(BigDecimal.valueOf(0.90)), coinBalance);// sell a little lower to ensure it is sold
                    orders.add(order);
                }
            ...
            if (botTradeConfig.getSellMode() == 1 && botTradeConfig.getSellOrderGiven() == 1 && lowestBuyPrice.compareTo(botTradeConfig.getBuyAtPriceHigh()) <= 0) {
                ...
                Order order = new Order(currPair, SELL_ACTION, botTradeConfig.getBuyAtPriceHigh(), coinBalance);// sell a little lower to ensure it is sold
                orders.add(order);
            ...
            if (botTradeConfig.getSellMode() == 0 && lowestBuyPrice.compareTo(botTradeConfig.getSellModePrice()) <= 0 && botTradeConfig.getBuyOrderGiven() == 0 && (openOrderMap.get(currPair) == null || openOrderMap.get(currPair).isEmpty())) {
                ...
                for (int i = 0; i < numberOfLowSplits; i++) {
                    ...
                    Order order = new Order(currPair, BUY_ACTION, buyPrice, amountOfBalancePerBuy.divide(buyPrice, new MathContext(5)));
                ...
                Order order = new Order(currPair, BUY_ACTION, botTradeConfig.getBuyAtPriceLow(), amountOfBalanceLeft.divide(botTradeConfig.getBuyAtPrice(), new MathContext(5)));
                orders.add(order);
            ...
            if (botTradeConfig.getSellMode() == 0 && botTradeConfig.getBuyOrderGiven() == 1 && botTradeConfig.getSellOrderGiven() == 0 && lowestBuyPrice.compareTo(botTradeConfig.getSellModePrice()) > 0 && coinBalance.compareTo(BigDecimal.valueOf(0)) > MIN_COIN_BUY_AMOUNT) {//switch to sell mode
                ...
                for (int i = 0; i < numberOfLowSplits; i++) {
                    ...
                    Order order = new Order(currPair, SELL_ACTION, sellPrice, amountOfCoinsPerSell);
                ...
                Order order = new Order(currPair, SELL_ACTION, botTradeConfig.getSellAtPriceLow(), amountOfCoinsLeft);
                orders.add(order);
    ...
    private void cancelOpenOrders(BinanceTradingApi tradingApi, Map<String, List<PoloniexOpenOrder>> openOrderMap, String currPair) {
        //cancel all orders
